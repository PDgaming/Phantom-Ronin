...
func drawSettingsMenu(g *Game) {
	...
	if g.state.ShowIntro != initialShowIntro {
		err := SaveSettings(SettingsData{ShowIntro: g.state.ShowIntro, UseJiraiyaModel: g.state.UseJiraiyaModel, UsePlatformModel: g.state.UsePlatformModel})
		if err != nil {
	...
	backButton := gui.Button(rl.NewRectangle(float32(screenWidth)/2-50, 350, 100, 40), "Back") // Adjusted Y position
	if backButton {
